...
def DE_crossover(par1_vector: torch.Tensor, par2_vector: torch.Tensor, par3_vector: torch.Tensor, CR: torch.Tensor, F: torch.Tensor):
    """
    Performs binary crossover in differential evolution.
    ...
    :return: The trial vector after crossover for each individual.
    """
    mutation_vector = par1_vector + F * (par2_vector - par3_vector)
    current_vector = par1_vector
    device = par1_vector.device
    pop_size, dim = mutation_vector.size()
    if CR.ndim == 1:
        CR = CR.unsqueeze(1)
    mask = torch.randn(pop_size, dim, device=device) < CR
