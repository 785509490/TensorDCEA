...
class EvalMonitor(Monitor):
    ...
    constraints_history: List[torch.Tensor]
    ...
    def __init__(
        ...
        full_constraints_history: bool = False,
        topk: int = 1,
        ...
        self.full_constraints_history = full_constraints_history
        ...
        self.latest_constraints = Mutable(torch.empty(0, device=device))
    ...
    def set_config(self, **config):
        ...
        if "full_constraints_history" in config:
            self.full_constraints_history = config["full_constraints_history"]
        if "topk" in config:
    ...
    def pre_tell(self, fitness: torch.Tensor):
        if isinstance(fitness, tuple):
            ...
            constraints = fitness[1]
            fitness = fitness[0]
            self.latest_constraints = constraints
