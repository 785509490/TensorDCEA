...
class DNSGA2A(Algorithm):
    ...
    def step(self):
        ...
        #环境响应策略
        if is_changed:
            print(f"Change detected. Re-initializing {self.reinit_ratio * 100}% of the population.")
            # 2. If changed, re-initialize a portion of the population
            self.reinitialize_population(self.reinit_ratio)
            self.init_step()
    ...
    def reinitialize_population(self, ratio: float):
        """Re-initializes a random portion of the population's decision variables."""
        n_reinit = math.ceil(self.pop_size * ratio)
        if n_reinit <= 0:
            return

        indices = torch.randperm(self.pop_size, device=self.pop.device)[:n_reinit]

        length = self.ub - self.lb
        new_decs = self.lb + length * torch.rand(n_reinit, self.dim, device=self.pop.device)
