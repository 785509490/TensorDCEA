...
class DNSGA2B(Algorithm):
    ...
    def step(self):
        ...
        #环境响应策略
        if is_changed:
            print(f"Change detected. Re-mutating {self.reinit_ratio * 100}% of the population.")
            # 2. If changed, re-mutate a portion of the population
            self.remutate_population(self.reinit_ratio)
            self.init_step()
    ...
    def remutate_population(self, ratio: float):
        """Re-mutates a random portion of the population."""
        n_remutate = math.ceil(self.pop_size * ratio)
        if n_remutate <= 0:
            return

        # 随机选择需要变异的个体索引
        indices = torch.randperm(self.pop_size, device=self.pop.device)[:n_remutate]

        # 对选中的个体进行变异
        mutated_decs = self.mutation(self.pop[indices], self.lb, self.ub)
        mutated_decs = clamp(mutated_decs, self.lb, self.ub)

        # 替换种群中的个体
        self.pop[indices] = mutated_decs
